...
- [unet.cu](#unetcu)
	...
	- [Version 1: naive implementation](#version-1-naive-implementation)
		...
		- [New kernels: up/down samples and convolutions](#new-kernels-updown-samples-and-convolutions)
...
To train a diffusion model in CUDA with the default elephant images, run the following:
...
To run the CUDA model with your own data, you need to create a `.bin` file with the image data first:

```bash
python prepare_data.py --data_dir YOUR_DATA_DIR --output_name YOUR_BINARY_DATA_FILENAME.bin
# now run training, assuming you had already initialized the model as above
...
Inspired by Andrej Karpathy's [llm.c](https://github.com/karpathy/llm.c), I decided to build a UNet from scratch in C++/CUDA. The goal of the project is to learn the concepts in llm.c, and ultimately to try to reach PyTorch's performance with our CUDA implementation. I chose the UNet because it is a key architecture for diffusion models, and I will do some simple diffusion model training with the model.

Diffusion model training is quite sophisticated nowadays. Since this project is focused on learning CUDA as opposed to building the best diffusion model, I prioritized simplicity over performance, and followed the implementation from the paper [Diffusion Models Beat GANs on Image Synthesis](https://arxiv.org/abs/2105.05233). Currently the UNet only supports unconditioned diffusion training. I also did not reproduce all the model configurations from the paper; the details of the differences will be explained in the next section.

Here are some images generated with our CUDA implementation. The model is trained on elephant images from ImageNet 64x64 without class-conditioning. The model seems to be highly over fitting the training set right now, but at least this confirms training is working.
...
The rest of these notes are organized as follows. The next section will cover some background, both on diffusion models and on the specific UNet architecture we use. Then the later sections document successive iterations of the model where I benchmark the kernels and try to speed things up. It turns out that most of a UNet's running time is spent doing `3x3` image convolutions, so that is where most of the work went into and where these notes focus the most on.

## Background

### Diffusion models

Our goal is to train a diffusion model with a UNet. Let me give a short summary of how diffusion models work; a good mathematical description can be found in Appendix B of the paper [Diffusion Models Beat GANs on Image Synthesis](https://arxiv.org/abs/2105.05233). We start with a target distribution  $\pi(x)$ on $\mathbb{R}^d$ that we want to sample from. In our case, the space will be RGB images with $C = 3$ channels and height and weight $H = W = 64$, so $d = C \times H \times W$, and the target distribution will be elephant images. The key idea is to then set up a stochastic process $(X_t)_{t \ge 0}$ with the following three properties:

1. At $t = 0$, $X_0$ are exact samples from $\pi(x)$.
2. When $t$ is very large, then $X_t$ is very close in distribution to the standard Gaussian distribution on $\mathbb{R}^d$.
...
So now all we're missing is a stochastic process that satisfies these properties, and a way to learn the conditional distributions in property 3. The stochastic process $(X_t)_{t \ge 0}$ will look like so: $X_0$ is drawn from $\pi$, and $X_t$ is given as follows:

$$
X_t = \sqrt{\alpha_t} \cdot X_0 + \sqrt{1 - \alpha_t}\cdot \epsilon,
$$

where $\epsilon$ is a standard Gaussian in $\mathbb{R}^d$, and $\alpha_t$ is a non-increasing function of $t$ that we will choose, with the properties that $\alpha_0 = 1$ and $\alpha_t \to 0$ as $t \to \infty$. We see that when $t$ is large, $X_t \approx \epsilon$, which satisfies property 2. Finally, to sample from the conditional distribution $\pi(X_{t-1} \mid X_t)$, we will train a model $\epsilon_\theta(X_t, t)$ that takes $X_t$ and $t$ as input, which minimizes the following objective:
...
A few more notes on the model details:

- During the upsample blocks, we concatenate the skip connections to form the inputs.
- To do diffusion model training, we also need to take in time step embeddings.
	- We construct sinusoidal embeddings for the time steps, as in the paper.
	- Then we pass the embeddings through a fully connected layer, and add the embeddings to the input of each Residual block.
- We did not implement dropout.
...
- The full code for our UNet can be found in `unet.py`. Our model exactly matches the [official implementation](https://github.com/openai/guided-diffusion) with the following model configurations:
...
In the first version I wanted to get something working quickly, so I tried to copy or adapt the kernels from llm.c. This approach took care of some kernels: the linear layer could be reused from llm.c without adaption; the groupnorm layer is different from the layernorm in llm.c, but we only needed to change the axis we reduce over and then we had a working kernel.

The self-attention layer was trickier to adapt. At first glance, the attention layer should function identically for transformers and image models (we won't be using a causal mask for images, but that barely affects the algorithm). However, there is a key difference in how text and image data are stored. When fed into an attention layer, text data usually takes the shape `(B, T, C)`, for the batch, context, and channel dimensions, whereas image data takes the shape `(B, C, H, W)`, for the batch, channel, height and width dimensions. In both cases, the attention layer then uses the `C` dimension to create QKV vectors.

In the first pass, I took the following approach with the attention layer:

1. transpose the inputs to shape `(B, H x W, C)`
2. then used the attention kernels from llm.c with a context size of `H x W`, and get an output of shape `(B, H x W, C)`,
3. then transposed the output back to shape `(B, C, H, W)`.
This turns out to be highly inefficient, because for each transpose we need to move a block of size `B x C x H x W` in and out of GPU global memory. Moving lots of data in and out of global memory should definitely cause some alarm (more on this later), so this should be an obvious place of improvement in future iterations.

### New kernels: up/down samples and convolutions

Several kernels did not exist in llm.c, but they are needed for the UNet. They are:

- Up and down sample,
- Convolutions with filter sizes 3 and 1.
The up and down sample kernels (nearest interpolation and average pooling respectively) are easy: there's barely any computation, and we easily parallelize operations by assigning one pixel to each GPU thread.
...
This plan turns out to be quite natural for the `1x1` convolution: for inputs of shape `(B, C_in, H, W)` and weights of shape `(C_out, C_in)`, the forward pass for a `1x1` convolution is essentially a matrix multiplication in the `C_in` dimension of the input with the weights. So `1x1` convolutions consist of the following steps:

1. transpose the input from `(B, C_in, H, W)` to `(B x H x W, C_in)`,
2. do a single matrix multiplication of the input with the weights with cuBLAS to get an output of shape `(B x H x W, C_out)`, then add the bias,
3. transpose the output back to shape `(B, C_out, H, W)`.
Notice again that this approach needs two transposes of the entire input array, which are expensive. In iteration 2 we will write a custom kernel that avoids the transposes altogether.
...
- output: `(B, C_out, H, W)`.
Since the plan is to cast the convolution into a matmul, it seems natural to transpose the input and output to shapes `(B x H x W, C_in)` and `(B x H x W, C_out)` respectively. As for the weight tensor, we can actually think of it as consisting of 9 different weight matrices, all of shape `(C_out, C_in)`, where each one corresponds to one out of the 9 filters used in the `3x3` convolution. Let's introduce some notation: let the transposed input be $X \in \mathbb{R}^{(B\cdot H \cdot W) \times C_\text{in}}$, the transposed output be $Y \in \mathbb{R}^{(B\cdot H\cdot W) \times C_\text{out}}$, and let the weight tensor be $W\in \mathbb{R}^{C_\text{out} \times C_\text{in} \times 9}$, where $W = (W_0, \dots, W_8)$, and each $W_i \in \mathbb{R}^{C_\text{out} \times C_\text{in}}$ is the weight matrix for filter $i \in \{0, \dots, 8\}$.

The convolution of a single pixel works by multiplying the pixel value with the 9 filters and summing the values. So roughly speaking, the convolution for the entire batch looks something like this:

1. For each filter $i$, multiply the input and the weight by $X W_i^\intercal$ and obtain an output of shape `(B x H x W, C_out)`.
2. Sum over the filters and obtain the transposed output $XW_0^\intercal  + \dots XW_8^\intercal$.
So we have turned the `3x3` convolution into a matrix multiplication.

Except this is not quite right, because not every pixel is multiplied with every filter. For instance, if we think of the pixel in the top left pixel of the input, it will only ever be multiplied with weights from the filters 0, 1, 3 and 4, as shown in this sketch.

<p align="center" id="pixel-filter">
  <img src="assets/pixel_to_filter.png" width="70%" />
</p>

The correct solution will have to account for the pixel-filter combinations that never occur in the output. It can be roughly described as follows:

1. Create a tiled input matrix `X_tilde` of shape `(B x H x W, 9 x C_in)`, such that the values in `X_tile[:, i*C_in : (i+1)*C_in]` are values that will be multiplied with filter $i$. If an input pixel is never multiplied with any of the filters, then the corresponding values for that pixel in `X_tilde` will be filled with zeros.
2. Treat $W$ as a `(C_out, C_in x 9)` matrix, and do a single matrix multiplication `X_tilde @ W.T` to get the transposed output of shape `(B x H x W, C_out)`.
...
Just looking at this algorithm should make us a bit nervous: we will have to be reading/writing arrays of size `9 * B * C_in * H * W` in and out of global memory, which is definitely not optimal. But at least we now have a fully working version of all the kernels, so let's see how fast our UNet is.

### Benchmarking the first kernel

So we have a working forward and backward pass for the UNet. Let's compare its performance to the same model in PyTorch. These benchmarks are run on an RTX 4090:
...
In these benchmarks the sum of the times for the convolutions actually slightly exceed the total time of the residual block, which is likely due to the extra `cudaEventSynchronize` calls we make when benchmarking the convolution kernels separately. The point here is that most of the time in residual blocks is spent in the convolution kernels. So what is taking so long in the convolution kernels? Here is a breakdown for the forward pass of the `3x3` convolution with `C_in = 192`, `C_out = 64`, `H = 64` and `W = 64`:

| Operation | time (ms) |
| --- | --- |
| Conv `3x3` `C_in = 192` forward | 15.1033 |
...
Second, a significant amount of time is spent transposing and filling in the `X_tilde` matrix, which is only needed because we are trying to shoehorn the convolution through a matmul. If we could avoid this expensive memory transfer step, we could hope to reduce the time taken by the entire kernel to just the time taken by the cuBLAS SGEMM step, which is doing all the computation. In fact, it turns out that PyTorch convolution is even faster than the SGEMM step above:
...
This makes sense, beecause `X_tilde` essentially consists of the input $X$ tiled $9$ times, which means that even for the computation step we need to move more memory in the GPU.

The story for the backward pass is very similar: most of the UNet backward pass is spent in residual block backward passes:
...
The convolution backward pass is again wasting time allocating buffer memory and moving a lot of data, thought the fraction of time spent doing computations is higher than in the forward pass (also with `C_in = 192`, `C_out = 64`, `H = 64`, `W = 64`):
...
and $(h, w)$ will be multiplied with filter $i$ if the output pixel is in range of the image. So if we want to compute the contribution to the output channel $o$ from the input pixel $(h, w)$ and filter $i$, we need to take a dot product between the $h W + w$ column of the input matrix, with elements at indices $9 c + i$ on row $o$ for the weight matrix, for $c = 0, \dots, C - 1$, as shown in the following sketch.
...
Usually a CUDA kernel functions like so:

- You start with all the relevant data in GPU global memory.
- Because SMEM is much faster than global memory, you would load the relevant data for each block from global memory into SMEM before doing any computation.
...
- Essentially all the work goes into making sure that the kernels are always doing computations and seldom stalling to wait for memory loads.
- When loading from global memory, you want to make sure that adjacent threads are loading adjacent memory locations, so global memory access can be coalesced and loads are much faster.
	- The same thing applies to SMEM loads: memory in SMEM should be organized such that adjacent threads only need to load memory from adjacent locations.
- Having many threads trying to load from SMEM can result in stalling as well. So instead of relying on SMEM for computations, you actually want to load data first into thread registers, and then use registers to do most of the computation in the threads.

### Kernel details

With these points in mind, we can write our `3x3` convolution kernel. We will be roughly following [kernel 5](https://github.com/siboehm/SGEMM_CUDA/blob/master/src/kernels/5_kernel_2D_blocktiling.cuh) of Simon's blog, since the convolution looks [roughly like a Matmul](#conv-computation). As mentioned [earlier](#3x3-convolution-in-detail), we will work with a weight matrix of shape `(O, 9 * C)`, an input of shape `(C, H * W)`, and produce an output of shape `(O, H * W)`. Each block will fill out a `(BO, BH)` shaped subset of the output, and each thread within each block will fill out a `(TO, TH)` shaped subset of the block's output, as shown below.

<p align="center" id="conv-computation">
  <img src="assets/conv_kernel_overview.png" width="75%" />
</p>

For each block, we loop over the `C` axis on which the dot products are taken. In each iteration, each block loads a `(BO, 9 * BC)` chunk of the weights and a `(BC * BH)` chunk of the inputs into SMEM. Then each thread will loop over the `BC` axis of the SMEM arrays. In each iteration, each thread loads a `(TO, 9)` chunk of the SMEM weights and `(1, TH)` chunk of the SMEM inputs into local registers, and computed the appropriate product. This process is illustrated below.
...
After also writing a custom kernel to avoid the transpose in the `1x1` convolutions, the forward pass is now at about 2/3 the speed of PyTorch and slightly 45% of the speed of PyTorch Nightly with `torch.compile`:
...
And here are the full training loop times, reproduced from the start of these notes:
...
And just as before, most of the time of the residual blocks are spent in the `3x3` convolutions. So to reach PyTorch's performance we simply have to optimize the convolution kernels.

As mentioned in an [earlier section](#profiling-version-2), our current kernel is blocked on loading large chunks of the input matrix into SMEM, but if we try to reduce the size of the SMEM buffer, we need to spin up more blocks and threads, which makes the atomic update to the outputs slow. So the main ideas are ways to avoid atomic writes to the output buffer.

I can think of two approaches. The first approach is to run the convolution in a single kernel. For each thread, we would load all the input pixels needed to compute the full outputs, so different threads will write in disjoint output locations. This approach would result in repeated memory loads though, because to compute the convolution outputs on a block of pixels requires knowing the input of a larger block of pixels. The pixels that are loaded repeatedly are referred to as the apron. Figure 4 from the tutorial [Image Convolution with CUDA](https://developer.download.nvidia.com/compute/cuda/1.1-Beta/x86_64_website/projects/convolutionSeparable/doc/convolutionSeparable.pdf) illustrates the apron in yellow:

<p align="center" id="apron-img">
  <img src="assets/conv_apron.png" width="40%" />
</p>

Because of the apron, one of the blocks we load or write will not have a width that is a multiple of 32, which makes it challenging to have well aligned read/write operations.

The second approach is to keep the kernel mostly the same as now, but do non-atomic writes to a global memory buffer, which will then be reduced to the final output after synchronization or in a separate kernel. We avoid the challenges of dealing with the apron, but this does result in more memory reads and writes.

I don't have any intuition on which of these will be faster yet, so I'll have to try them.

### Backward pass

Optimizing the backward pass, like that for the forward pass, consists almost entirely of optimizing the `3x3` convolution backward pass. We didn't discuss the backward pass in detail in these notes, but on a high level it works like so. We need to compute three things: `dinput`, `dweight`, and `dbias`. `dbias` is easy; `dweight` is computed using the input and `dout`, the gradient of the output; `dinput` is computed using the weight and `dout`.
...
As for computing `dweight`, we need to deal with the issue of the [apron](#apron-img). Currently there is a lot of inefficiency in the memory loads for the `dweight` kernel, and I need to rethink the algorithm here.

Another idea is to try to compute `dinput` and `dweight` in the same kernel, because the `dout` matrix is used to compute both, so it might save time to load it in once. This is potentially challenging though, because we are multiplying different axes of `dout` in the two computations.

## Acknowledgements

I was inspired to work on this project by Andrej Karpathy's [llm.c](https://github.com/karpathy/llm.c) and Simon Boehm's block, [How to Optimize a CUDA Matmul Kernel for cuBLAS-like Performance: a Worklog](https://siboehm.com/articles/22/CUDA-MMM). I learnt a tremendous amount from those projects.
